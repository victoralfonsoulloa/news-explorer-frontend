...
import { SavedArticlesContext, SavedArticle, SavedArticlesContextType } from './SavedArticlesContext';
...
export default function SavedArticlesProvider({ children, userId }: SavedArticlesProviderProps) {
  ...
  const removeArticle = async (articleId: string): Promise<void> => {
    return new Promise((resolve, reject) => {
      setTimeout(() => { // Simulate API delay
        try {
          setSavedNews(prev => prev.filter(article => article._id !== articleId));
          resolve();
        } catch (error) {
          reject(error);
        }
